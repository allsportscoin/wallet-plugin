...
// send type drop
const OPEN_TYPE_DROPDOWN = 'metamask/send/OPEN_TYPE_DROPDOWN';
const CLOSE_TYPE_DROPDOWN = 'metamask/send/CLOSE_TYPE_DROPDOWN';
...
export default function reducer ({ send: sendState = initState }, action = {}) {
	...
	switch (action.type) {
		...
		case OPEN_TYPE_DROPDOWN:
			return extend(newState, {
				typeDropdownOpen: true,
			});
		case CLOSE_TYPE_DROPDOWN:
			return extend(newState, {
				typeDropdownOpen: false,
			});
...
export function openTypeDropdown () {
	return { type: OPEN_TYPE_DROPDOWN };
}

export function closeTypeDropdown () {
	return { type: CLOSE_TYPE_DROPDOWN };
}
